...
void Lab7::RenderSimpleMesh(Mesh* mesh, Shader* shader, const glm::mat4& modelMatrix)
{
    ...
    glUniformMatrix4fv(shader->GetUniformLocation("Bones"), 200, GL_FALSE, glm::value_ptr(bones[0]));
...
unsigned int Lab7::FindRotation(float animationTime, const aiNodeAnim* pNodeAnim)
{
    ...
    unsigned int last = 0;
    for(int i = 1; i < pNodeAnim->mNumRotationKeys; i++)
    {
...
void Lab7::ComputeInterpolatedRotation(aiQuaternion& out, float animationTime, const aiNodeAnim* pNodeAnim)
{
    ...
    out.Interpolate(out, pNodeAnim->mRotationKeys[rotationIndex].mValue, pNodeAnim->mRotationKeys[nextRotationIndex].mValue, factor);
}
...
unsigned int Lab7::FindScaling(float animationTime, const aiNodeAnim* pNodeAnim)
{
    ...
    unsigned int last = 0;
    for (int i = 1; i < pNodeAnim->mNumScalingKeys; i++) {
...
void Lab7::ComputeInterpolatedScaling(aiVector3D& out, float animationTime, const aiNodeAnim* pNodeAnim)
{
    ...
    // Save the result in the out variable
    out = pNodeAnim->mScalingKeys[scalingIndex].mValue + (pNodeAnim->mScalingKeys[nextScalingIndex].mValue - pNodeAnim->mScalingKeys[scalingIndex].mValue) * factor;
}
...
unsigned int Lab7::FindPosition(float animationTime, const aiNodeAnim* pNodeAnim)
{
    ...
    unsigned int last = 0;
    for (int i = 1; i < pNodeAnim->mNumPositionKeys; i++) {
...
void Lab7::ComputeInterpolatedPosition(aiVector3D& out, float animationTime, const aiNodeAnim* pNodeAnim)
{
    ...
    out = pNodeAnim->mPositionKeys[positionIndex].mValue + (pNodeAnim->mPositionKeys[nextPositionIndex].mValue - pNodeAnim->mPositionKeys[positionIndex].mValue) * factor;
}
