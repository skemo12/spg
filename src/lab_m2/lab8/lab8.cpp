...
void Lab8::Init()
{
    ...
    originalImage = TextureManager::LoadTexture(
        PATH_JOIN(window->props.selfDir, RESOURCE_PATH::TEXTURES, "cube", "pos_x.png"),
    ...
    std::string shaderPath =
        PATH_JOIN(window->props.selfDir, SOURCE_PATH::M2, "Lab8", "shaders");
...
void Lab8::FrameEnd() { DrawCoordinateSystem(); }
...
void Lab8::GrayScale(unsigned char *data)
{
...
void Lab8::Blur(unsigned char *data)
{
    unsigned int channels = originalImage->GetNrChannels();
    // unsigned char* data = originalImage->GetImageData();
    unsigned char *newData = processedImage->GetImageData();
...
void Lab8::Sobel()
{
    unsigned char *data = originalImage->GetImageData();
    Blur(data);
    GrayScale(processedImage->GetImageData());
    unsigned char *newData = processedImage->GetImageData();
    ...
    for (int i = 1; i < imageSize.y - 1; i++)
    {
        ...
        {
            int a_0_0 = data[3 * ((i - 1) * imageSize.x + j - 1)];
            int a_0_1 = data[3 * ((i - 1) * imageSize.x + j)];
            int a_0_2 = data[3 * ((i - 1) * imageSize.x + j + 1)];
            int a_1_0 = data[3 * (i * imageSize.x + j - 1)];
            int a_1_2 = data[3 * (i * imageSize.x + j + 1)];
            int a_2_0 = data[3 * ((i + 1) * imageSize.x + j - 1)];
            int a_2_1 = data[3 * ((i + 1) * imageSize.x + j)];
            int a_2_2 = data[3 * ((i + 1) * imageSize.x + j + 1)];
            ...
            if (val < 250)
                val = 0;
            newData[3 * (i * imageSize.x + j)] = (unsigned char) val;
            newData[3 * (i * imageSize.x + j) + 1] = (unsigned char) val;
            newData[3 * (i * imageSize.x + j) + 2] = (unsigned char) val;
        }
    }
    processedImage->UploadNewData(newData);
}
...
void Lab8::OnKeyPress(int key, int mods)
{
    ...
    if (key - GLFW_KEY_0 >= 0 && key <= GLFW_KEY_4)
    {
        ...
        {
            if (outputMode == 1)
                GrayScale(originalImage->GetImageData());
            if (outputMode == 2)
                Blur(originalImage->GetImageData());
            if (outputMode == 3)
                Median();
            if (outputMode == 4)
                Sobel();
